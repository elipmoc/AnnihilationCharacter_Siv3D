...
namespace  game {
	...
	{
		if (count++ == 0) {
			GetBulletList().MakeBullet(GetPos(), { -4.5,0 }, { 0,0 });
			GetBulletList().MakeBullet(GetPos(), { -4.5 * siv::Math::Cos(siv::Math::Radians(30)),-4.5 * siv::Math::Sin(siv::Math::Radians(30)) }, { 0,0 });
			GetBulletList().MakeBullet(GetPos(), { -4.5 * siv::Math::Cos(siv::Math::Radians(-30)),-4.5 * siv::Math::Sin(siv::Math::Radians(-30)) }, { 0,0 });
			siv::SoundAsset(L"shot").playMulti();
		}
	}
